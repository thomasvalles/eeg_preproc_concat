...
str_DirInput = '/Users/pkornman/Desktop/UCLA_EEG/T_Input/';
str_DirOutput = '/Users/pkornman/Desktop/UCLA_EEG/Test_Output/';
...
if any(contains(ListFiles,'During1'))
    during1=1;
else
    during1=0;
end
...
for i = 1:numel(ListFiles)
    ...
    elseif during1==1
        ...
        elseif(strcmp(tmpFile{2},'During') )
            time = 2;
        elseif(strcmp(tmpFile{2},'During1')  )  
            time =3;
    ...
    for j = 1:numel(ListFiles(:,1))
        if(tmpSubjID(j, 1) == subjID(i,1) && tmpSubjID(j, 4) == subjID(i,2) && tmpSubjID(j, 3) == subjID(i,3))
            ...
            elseif(tmpSubjID(j,5) == 3)
              ...
              elseif(tmpSubjID(j,5) == 4)
                ListConcatFiles_temp{4,1} = ListFiles{j,1};  
            end
        end
    end
...
%             [ EEG, str_ChanLab ] = f_CleanEDFLabels( EEG );
        ...
        EEG=pop_chanedit(EEG, 'lookup','/Users/pkornman/Desktop/UCLA_EEG/CODE/eeglab2021.1/plugins/dipfit/standard_BESA/standard-10-5-cap385.elp');
        
        
        
        %% fix channel numbers of each EEG files if necessary before merging 
        if(EEG.nbchan == 63)
