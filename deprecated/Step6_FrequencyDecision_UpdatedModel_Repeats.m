...
dirData = [work_dir, subjid, '/Step6_ASCOutput/'];
% %Where to save output mat files
saveDir = [work_dir, subjid, '/Step7_FrequencyOutputDecision/'];
mkdir(saveDir);
...
%     AllStimFreq{1}(1) = AllStimFreq{1}(2);
    ...
    UniqueVals_sorted = nan(numel(UniqueFreqs), numel(EveryStimFreq) / numel(UniqueFreqs), size(AllPostData{iPat,1},3));
    ...
    txt = ['* : RF max' newline '/ : RF min'] ;
    yl = ylim;
    text(17,yl(2) - 5,txt, 'FontSize', 10)

    xlabel(['RF Maxes: ', mat2str(m2_maxs), '    RF Mins: ', mat2str(m2_mins)], 'FontSize', 15);
