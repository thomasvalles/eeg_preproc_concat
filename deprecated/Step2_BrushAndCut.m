...
%ElectrodeLocation = '~/Documents/interrogation_processing/standard-10-5-cap385.elp';
...
if nPat >= 0
    ...
    %EEG = pop_loadset('/Volumes/4TB WD Elements/work_dir/InterrogationProtocol/SSCPipeline/21-180_2022-01-10/Step2_MergedSetFiles/test/rmbase.set')   
        ...
        if(numel(EEG.event) > 0)
            ...
            %     shiftedTriggers = pulseTriggers - 6;% - 6;
            % end
            %shiftedTriggers = pulseTriggers - (17 * (EEG.srate/1000));%magstim
    %         
...
for iPat = 1:nPat
    ...
    while(flag_redo == 0)
        ...
        if (numel(trigInterrog) < 1000)
            ...
            option = input('Which option would you like to use for trigger detection[1|2|3]?');
            [trigInterrog] = f_DetectTriggersManually(F3,srate, [brushX(1) brushX(end)], option);
            
            %magstim problem??
            % if contains(subjid, 'BH')
            %     %trigInterrog = trigInterrog - (17 * (EEG.srate/1000));
            %     trigInterrog = trigInterrog - 6;
            % end
