...
clc;
clear;
addpath '/Users/neuromodit/Documents/RF_StepsMatlab'
...
work_dir = '/Volumes/Files/Interrogation/Training_OldSubjs/';
subdirs = dir(fullfile(work_dir, '*'));
subdir_names = {subdirs.name};
subdir_names_filtered = subdir_names(~startsWith(subdir_names, '.'));
subdir_names_filtered = subdir_names_filtered(~contains(subdir_names_filtered,'Butler'));
for isub = 1:numel(subdir_names_filtered)
    
    subj = subdir_names_filtered(isub);
    subj = char(subj);
    if ismember(string(subj(1)),["S","1","2","0"])
    dates = dir([work_dir, char(subj)]);
    dates = {dates.name};
    dates = dates(~startsWith(dates, '.'));
    for idate = 1:numel(dates)
        clearvars -except isub subdir_names_filtered subj dates idate work_dir
        subjid = [char(subj), filesep(), char(dates(idate))]


        % subjid = '18-087/18-087_20181008';
...
%         if isempty({dir([saveDir 'Pow*Py*.mat']).name})
        if ~isdir(saveDir)
            mkdir(saveDir)
        end
        ...
        files = dir('*.set');
        files = {files.name}';
        files = files(~startsWith(string(files),'._'));
        patID = [];
        for iFile = 1:numel(files)
            tmpsplit = strsplit(files{iFile}, '_');
            patID{iFile} = files{iFile}(1:end - 4);
        end
        nPat = numel(files);
        ...
        for iPat = 1:nPat
            ...
            if ~ (numel(stimFreqMedian) == size(EEG.data,3))
                error('Mismatch Triggers, EEG')
            end 
            ...
            % make a tighter version with only f3 seed for faster loading
            seed =5 ;
