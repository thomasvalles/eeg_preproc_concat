function [ m_TFPRE,m_TFPOST, v_FreqAxis, outpar ] = f_TF_SCCEntrainment( v_Sig, TOIBL,TOIPost, fmin, fmax, s_StDevCycles,  srate, alg, s_FreqSeg) % 'abs', 'zs', 'zsn', '01'
% algorithm input: 
% 'abs' = absolute
% 'zs' = zscore
...
for iChan = 1:size(v_Sig,1)
    ...
    %%ABSOLUTE%%%%%%%%%%%%%%%
    %v_PSD_PRE(:,iChan) = mean(m_GaborWT(:,TOIBL),2);
    %v_PSD_POST(:,iChan) = mean(m_GaborWT(:,TOIPost),2);
    %%ABSOLUTE%%%%%%%%%%%%%%%
