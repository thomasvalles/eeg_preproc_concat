...
% order (Hz units)
% v_StDevArray: Array containing the standard deviations for each
% wavelet (Gaussian window) used in the transform
% 
% Author: Mario Valderrama
%  
function [m_GaborWT, v_TimeAxis, v_FreqAxis, v_StDevArray] = ...
    ...
    [pv_Signal, pad_size] = f_Padding(pv_Signal, ps_PadPercent);
...
%     tic
    ...
    clear m_GaborWT v_StDevArray
    m_GaborWT = zeros(numel(v_FreqAxis), numel(v_TimeAxis));
    if nargout >= 4
        v_StDevArray = zeros(numel(v_FreqAxis), 1);
    else
        v_StDevArray = [];
    end
    s_FreqInd = 0;
    ...
    for s_FreqCounter = v_FreqAxis
        ...
        if ~isempty(v_StDevArray)
            v_StDevArray(s_FreqInd) = s_StDevSec;
        end
        
        if s_SampAve > 1
            clear v_GaborTemp
            v_GaborTemp = zeros(s_Len + (s_SampAve - 1), 1);
...
%     toc
    ...
    if s_IsEven
        m_GaborWT = horzcat(m_GaborWT, m_GaborWT(:, end));
    ...
    m_GaborWT = m_GaborWT(:, pad_size + 1 : end - pad_size - 2);
    v_TimeAxis = (0:size(m_GaborWT, 2) - 1)./ ps_SampleRate;
